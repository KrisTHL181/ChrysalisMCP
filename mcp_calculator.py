...
class CalculatorASTValidator(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name):
            ...
            if not (
                ...
                and node.func.attr in _allowed_names["math"]
            ):
                raise ValueError(
                    f"Disallowed attribute access or function call: {ast.dump(node.func)}"
                )
        else:
            raise ValueError(
                f"Disallowed function call type: {type(node.func).__name__}"
            )
        for arg in node.args:
            self.visit(arg)
        for kw in node.keywords:
            raise ValueError(
                "Keyword arguments are not allowed in calculator expressions."
            )

    def visit_Name(self, node):
        if (
            node.id not in _allowed_names and node.id != "math"
        ):  # 'math' itself is allowed as a module name
    ...
    def visit_Attribute(self, node):
        ...
        if not (
            ...
            and node.attr in _allowed_names["math"]
        ):
...
async def calculate(name: str, arguments: dict) -> str:
    ...
    try:
        ...
        return json.dumps(
            {"error": f"Invalid expression or unsupported operation: {e}"}
        )
